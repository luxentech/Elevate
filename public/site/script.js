...
// المدمج مع نظام المصادقة ولوحة تحكم الإدارة
// =========================================
...
const Admin = {
    ...
    setStatus(status) {
        ...
        btnSoon.className = 'a-status-btn';
        btnHide.className = 'a-status-btn';
        btnShow.className = 'a-status-btn';
...
const App = {
  ...
  updateHomeStats() {
      ...
      // الأرقام الحقيقية فقط: لو مفيش فرص أو أعضاء يظهر 0
      const oppsCount = Array.isArray(cards)
        ? cards.filter(c => (c.status || 'visible') !== 'hidden').length
        : 0;
      const usersCount = Array.isArray(users)
        ? users.filter(u => !u.isAdmin).length
        : 0;

      if(statOpps) statOpps.textContent = oppsCount;
  ...
  injectNavbar() {
    ...
    let authHtml = loggedIn && user ? `
      ...
      <div class="nav-dropdown-wrapper custom-ui-dropdown">
        <button class="ui-user-btn" type="button">
          ...
          <div class="u-avatar-circle">${user.username.charAt(0).toUpperCase()}</div>
        </button>
    ...
    el.innerHTML = `
      ...
      <div class="mobile-nav">
        ...
        <a href="?view=profile"><i class="fas fa-user"></i><span>الملف الشخصي</span></a>
  ...
  renderProfile() {
      ...
      // ===============================================
      // منطق حساب الإدمن (واجهة احترافية Premium Dashboard)
      // ===============================================
      if (user.username === 'admin@elevate.com') {
          // إخفاء إحصائيات وعناصر المستخدم العادي
          if(document.getElementById('prof-normal-meta')) document.getElementById('prof-normal-meta').style.display = 'none';
          if(document.getElementById('prof-normal-stats')) document.getElementById('prof-normal-stats').style.display = 'none';
          if(document.getElementById('prof-normal-sidebar')) document.getElementById('prof-normal-sidebar').style.display = 'none';
          if(document.getElementById('tab-overview-btn')) document.getElementById('tab-overview-btn').style.display = 'none';
          if(document.getElementById('tab-saved-btn')) document.getElementById('tab-saved-btn').style.display = 'none';
          if(document.getElementById('prof-save-header-btn')) document.getElementById('prof-save-header-btn').style.display = 'none';
          
          // إظهار تبويب الإدارة
          const adminBtn = document.getElementById('tab-admin-btn');
          if(adminBtn) {
              adminBtn.style.display = 'block';
              this.switchProfTab('prof-admin-tab'); 
          }

          // تصميم فخم ومختلف كلياً لهيدر الأدمن
          const profHeader = document.getElementById('prof-header');
          if(profHeader) {
              profHeader.style.background = 'linear-gradient(135deg, #0f172a 0%, #1e1b4b 100%)';
              profHeader.style.border = '2px solid rgba(245, 158, 11, 0.4)';
              profHeader.style.boxShadow = '0 10px 40px rgba(0,0,0,0.5), inset 0 0 20px rgba(245, 158, 11, 0.1)';
          }

          if(document.getElementById('prof-name-disp')) {
              document.getElementById('prof-name-disp').innerHTML = 'المدير العام <i class="fas fa-crown" style="color:#fbbf24; font-size: 20px; margin-right: 10px; filter: drop-shadow(0 0 10px rgba(245,158,11,0.5));"></i>';
          }
          
          if(document.getElementById('prof-avatar-char')) {
              document.getElementById('prof-avatar-char').innerHTML = '<i class="fas fa-user-shield"></i>';
              document.getElementById('prof-avatar-char').style.background = 'linear-gradient(135deg, #f59e0b, #d97706)';
              document.getElementById('prof-avatar-char').style.border = '3px solid #fef3c7';
          }

          if(document.getElementById('prof-handle-disp')) document.getElementById('prof-handle-disp').textContent = 'Admin. Elevate';
          
          if(document.getElementById('prof-role-badge')) {
              document.getElementById('prof-role-badge').innerHTML = '<i class="fa-solid fa-star"></i> لوحة التحكم المركزية VIP';
              document.getElementById('prof-role-badge').style.background = 'linear-gradient(135deg, #f59e0b, #d97706)';
              document.getElementById('prof-role-badge').style.color = '#fff';
              document.getElementById('prof-role-badge').style.border = 'none';
              document.getElementById('prof-role-badge').style.boxShadow = '0 4px 15px rgba(245, 158, 11, 0.4)';
          }
...
function nsAppendMsg(role, text) {
  var msgs = document.getElementById('ns-ai-msgs');
  if (!msgs) return null;
  var div = document.createElement('div');
  div.className = 'ns-msg ' + (role === 'user' ? 'ns-user' : 'ns-bot');
  var bbl = document.createElement('div');
  bbl.className = 'ns-bbl';
...
window.nsSend = async function() {
  var inp = document.getElementById('ns-ai-input');
  if (!inp) return;
  var text = inp.value.trim();
  if (!text || window.nsBusy) return;
  inp.value = '';
  window.nsBusy = true;

  nsAppendMsg('user', text);
  window.nsHistory.push({ role: 'user', content: text });

  var typing = nsAppendMsg('bot', '...');

  try {
    var res = await fetch('/api/public/chat', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ messages: window.nsHistory.slice(-12) })
    });
    var data = await res.json().catch(function(){ return {}; });

    if (!res.ok || !data.reply) {
      var errMsg = 'حصل خطأ مؤقت، حاول تاني بعد شوية.';
      if (res.status === 429) errMsg = 'الضغط عالي على المساعد دلوقتي، جرب بعد دقيقة.';
      if (res.status === 402) errMsg = 'رصيد المساعد الذكي خلص مؤقتاً.';
      if (typing) typing.textContent = errMsg;
    } else {
      if (typing) typing.textContent = data.reply;
      window.nsHistory.push({ role: 'assistant', content: data.reply });
    }
  } catch (e) {
    if (typing) typing.textContent = 'تعذر الاتصال بالمساعد، تأكد من الإنترنت وحاول تاني.';
  } finally {
    window.nsBusy = false;
    var msgs = document.getElementById('ns-ai-msgs');
